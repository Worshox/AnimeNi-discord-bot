...
const postsUpdate = require("../config/posts-update.json");
...
module.exports = {
  ...
  async execute(client) {
    switch (botConfiguration.botActivity) {
      case "watching":
        client.user.setActivity(botConfiguration.botActivityDetails, {
    ...
    console.log(
      `Bot works! Logged as ${client.user.tag}, he is ${botConfiguration.botActivity} ${botConfiguration.botActivityDetails}`
    );
    ...
    setInterval(findNewPosts, 60_000);
    ...
    async function findNewVideos() {
      ...
      try {
        ...
        response = await axios(
          "https://animeni.pl/wp-json/wp/v2/posts?per_page=10&_embed",
      ...
      for (let i = newVideosCount; i >= 0; i--) {
        ...
        const fields = [
          {
            name: "Informacje:",
            value:
              he.decode(videoData.content.rendered.replace(/<[^>]+>/g, "")) ||
              "Brak szczegółów",
          },
          // {
        ...
        for (const videoPing in videoPings) {
          if (videoName.includes(videoPings[videoPing][0])) {
            ...
            if (
              videoPings[videoPing][2] !== undefined &&
              videoPings[videoPing][2] !== 1
            ) {
        ...
        const descripton = `${bold("Zapraszamy do oglądania!")} ${roleMention(
          videoPings.odcinki[1]
        )} ${extraRolePingID !== "" ? roleMention(extraRolePingID) : ""}`;
        ...
        if (!i) {
          ...
          const videoUpdateFile = path.resolve(
            __dirname,
            "../config/video-update.json"
          );
          fs.writeFile(
            videoUpdateFile,
    ...
    async function findNewPosts() {
      ...
      try {
        response = await axios(
          "https://animeni.pl/wp-json/wp/v2/blog?per_page=10&_embed",
          { headers: { "Accept-Encoding": "*" } }
        );
      } catch (error) {
        console.log("Nie udało się pobrać informacji o nowych postach");
        return;
      }

      let newPostsCount = -1;

      for (const singlePost of response.data) {
        if (singlePost.slug === postsUpdate.lastKnownPostSlug) break;

        newPostsCount++;
      }

      for (let i = newPostsCount; i >= 0; i--) {
        const postData = response.data[i];

        let postImageData = "https://animeni.pl/";
        if (postData.featured_image_data !== null) {
          postImageData = postData.featured_image_data.src;
        }

        const message = `${bold("Nowy wpis na blogu!")} ${roleMention(
          postsUpdate.postPingRoleID
        )}`;
        ...
        const channel = client.channels.cache.get(postsUpdate.postChannelID);
        await channel.send({ content: message, embeds: [postEmbed] });

        if (!i) {
          postsUpdate.lastKnownPostSlug = postData.slug;
          const postUpdateFile = path.resolve(
            __dirname,
            "../config/posts-update.json"
          );
          fs.writeFile(postUpdateFile, JSON.stringify(postsUpdate), (error) => {
            if (error) console.log(error);
          });
        }
      }
